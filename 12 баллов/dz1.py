...
def main():
    ...
    request.get(url, {"cookies":{"user": '67508b925f71c6ad39915b77db7946fa'}})
    while 'Секретный ключ:' not in request.text:
        request_data = parse(request, url)
        if request_data.type == "GET":
            request.get(request_data.url, request_data.parametrs)
        if request_data.type == "POST":
            request.post(request_data.url, request_data.parametrs)
